...
var  meta_projectile_scale :float = 1.0
...
func _ready() -> void:
	...
	MetaProgress.apply_to_projectile(self)
	# Применяем мета-масштаб снаряда
	if meta_projectile_scale != 1.0:
		scale = Vector2(meta_projectile_scale, meta_projectile_scale)
