...
func _physics_process(_delta: float) -> void:
	...
	# Если враг слишком далеко за краем экрана — телепортируем на случайный край камеры
...
func _check_offscreen_and_reposition() -> void:
	...
	# Половина видимой области + небольшой буфер
	var half_w := viewport_size.x * 0.5
	var half_h := viewport_size.y * 0.5
	# Порог вне экрана: чуть больше видимой области
	var off_threshold_x := half_w + 120.0
	var off_threshold_y := half_h + 120.0

	var diff := global_position - cam_center
	var is_offscreen = (abs(diff.x) > off_threshold_x or abs(diff.y) > off_threshold_y)

	if is_offscreen:
		# Телепортируем на случайный край экрана (снаружи, но близко к краю)
		var margin := 32.0   # насколько за краем появится враг
		var side := randi() % 4   # 0=верх, 1=низ, 2=лево, 3=право
		var new_pos: Vector2
		match side:
			0:  # верх
				new_pos = cam_center + Vector2(randf_range(-half_w, half_w), -(half_h + margin))
			1:  # низ
				new_pos = cam_center + Vector2(randf_range(-half_w, half_w),  (half_h + margin))
			2:  # лево
				new_pos = cam_center + Vector2(-(half_w + margin), randf_range(-half_h, half_h))
			3:  # право
				new_pos = cam_center + Vector2( (half_w + margin), randf_range(-half_h, half_h))
